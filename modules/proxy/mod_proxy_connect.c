...
/*
 * Need to free if return is NOT NULL. 
 */
char* get_redirect(char *conf, char *url) {

    if (conf == NULL || url == NULL || strlen(url) <= 0) return NULL;
    ...
    while (1) {
        ...
        if (ptr[strlen(ptr) - 1] == '\n') ptr[strlen(ptr)-1] = '\0';
        if (ptr[strlen(ptr) - 1] == '\r') ptr[strlen(ptr)-1] = '\0';


        chain_url = strdup(ptr);
...
/*
 * Handler for the "RedirCONNECT" directive 
 */ 
static const char *set_redir_conf(cmd_parms *parms, void *cfg, const char *arg)
{
    server_rec *s = parms->server;
    connect_conf *conf =
        ap_get_module_config(s->module_config, &proxy_connect_module);
                                 ...
                                 apr_port_t proxyport)
{
    ...
    int  is_redirect = 0;
    char *connect_url = url;

    if (c_conf->redir_conf != NULL) {
	  connect_url = get_redirect(c_conf->redir_conf, connect_url);
          if (connect_url == NULL) {
		  return ap_proxyerror(r, HTTP_BAD_REQUEST,
                             apr_pstrcat(p, "URI redirect error: ", url,
                                         NULL));
	  }
	  is_redirect = 1;
    }
    ...
    if (APR_SUCCESS != apr_uri_parse_hostinfo(p, connect_url, &uri)) {
        int err = ap_proxyerror(r, HTTP_BAD_REQUEST,
                             apr_pstrcat(p, "URI cannot be parsed: ", connect_url,
                                         NULL));
    	if (is_redirect && connect_url != NULL) free(connect_url);
	return err;
    }

    ap_log_rerror(APLOG_MARK, APLOG_DEBUG, 0, r, APLOGNO(01019)
                  "connecting %s to %s:%d", connect_url, uri.hostname, uri.port);
    ...
    /* Check if it is an allowed port if using AllowCONNECT */
    if (is_redirect == 0 && !allowed_port(c_conf, uri.port)) {
        return ap_proxyerror(r, HTTP_FORBIDDEN,
                             "Connect to remote machine blocked");
    }
...
static const command_rec cmds[] =
{
    ...
    AP_INIT_TAKE1("RedirCONNECT", set_redir_conf, NULL, RSRC_CONF, "The conf file for redirected CONNECT"),
    {NULL}
};
